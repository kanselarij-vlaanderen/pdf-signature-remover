...
async function deleteMuFile(virtualFileUri, physicalFileUri, graph=APPLICATION_GRAPH, updateFunction=update) {
  try {
    fs.unlinkSync(physicalFileUri.replace('share://', '/share/'));
  } catch (e) {
    console.log(e);
  }

  const updateString = `PREFIX mu: <http://mu.semte.ch/vocabularies/core/>
PREFIX nfo: <http://www.semanticdesktop.org/ontologies/2007/03/22/nfo#>
PREFIX dbpedia: <http://dbpedia.org/ontology/>
PREFIX dct: <http://purl.org/dc/terms/>
PREFIX nie: <http://www.semanticdesktop.org/ontologies/2007/01/19/nie#>
